...
namespace crg {
	...
	void crgicoico::addpurchase(account_name purchaser, asset crgbought, asset eospaid)
	{
		eosio_assert(configs(_self, _self).exists(), "Application account not configured.");
		require_auth(configs(_self, _self).get().application);
	...
	void crgicoico::apply(const account_name contract, const account_name act) {
	...
	void crgicoico::transferReceived(const currency::transfer &transfer, const account_name code) {
		eosio_assert(configs(_self, _self).exists(), "Application account not configured");
